...
namespace ImGui {
    auto operator*(const float value, const ImVec2& obj1) {
        return ImVec2(obj1.x * value, obj1.y * value);
    }

    auto operator+(const ImVec2& obj1, const ImVec2& obj2) {
        return ImVec2(obj1.x + obj2.x, obj1.y + obj2.y);
    }
    ...
    bool SliderScalar2D(char const* pLabel, float* fValueX, float* fValueY, const float fMinX, const float fMaxX, const float fMinY, const float fMaxY, float const fZoom /*= 1.0f*/)
    {
        ...
        if (ImGui::IsMouseHoveringRect(oRect.Min - vSecurity, oRect.Max + vSecurity) && ImGui::IsMouseDown(ImGuiMouseButton_Left))
        {
            ...
            *fValueY = fMinY + vCursorPos.y / (oRect.Max.y - oRect.Min.y) * fDeltaY;
        ...
        *fValueY = std::min(std::max(*fValueY, fMinY), fMaxY);
        ...
        float const fScaleY = (*fValueY - fMinY) / fDeltaY;
        ...
        char pBufferX[16];
        char pBufferY[16];
        ImFormatString(pBufferX, IM_ARRAYSIZE(pBufferX), "%.1f", *(float const*)fValueX);
        ImFormatString(pBufferY, IM_ARRAYSIZE(pBufferY), "%.1f", *(float const*)fValueY);
            ...
            ImGui::IsMouseDown(ImGuiMouseButton_Left))
        {
            ...
            *fValueX = fMinX + vCursorPos.x / (oRect.Max.x - oRect.Min.x) * fDeltaX;
    ...
    //        {
    //            float const fMaxDist = sqrt(Dist2(vDragStart, vDragEnd));
